...
function Connections() {
  ...
  </div> : <Empty title="Connections unavailable" text="The server did not return integration status." />}<section className="configuration-section" aria-labelledby="connection-configuration"><SectionHeading title="Connection configuration" meta="Provider keys are encrypted before persistent storage" /><h2 id="connection-configuration" className="sr-only">Connection configuration</h2>{reasoning.isLoading || grafana.isLoading ? <State title="Loading connection configuration…" /> : reasoning.isError ? <RequestState error={reasoning.error} retry={() => reasoning.refetch()} /> : grafana.isError ? <RequestState error={grafana.error} retry={() => grafana.refetch()} /> : <div className="configuration-grid"><ReasoningConfiguration initial={reasoning.data!} /><GrafanaConfiguration initial={grafana.data!} /></div>}</section></div>
}
