...
type ReasoningProfile = { provider: string; base_url: string; model: string; credential_ref: string; credential_available: boolean; json_mode: boolean; restart_required?: boolean }
...
function Connections() {
  ...
  return <div className="standard-page"><PageHeader title="Connections" subtitle="Configure server-managed integrations without storing credentials in the browser." />{result.isLoading ? <State title="Checking connections…" /> : result.isError ? <RequestState error={result.error} retry={() => result.refetch()} /> : result.data ? <div className="settings-list">
  ...
  </div> : <Empty title="Connections unavailable" text="The server did not return integration status." />}<section className="configuration-section" aria-labelledby="connection-configuration"><SectionHeading title="Connection configuration" meta="Secrets remain in server environment variables" /><h2 id="connection-configuration" className="sr-only">Connection configuration</h2>{reasoning.isLoading || grafana.isLoading ? <State title="Loading connection configuration…" /> : reasoning.isError ? <RequestState error={reasoning.error} retry={() => reasoning.refetch()} /> : grafana.isError ? <RequestState error={grafana.error} retry={() => grafana.refetch()} /> : <div className="configuration-grid"><ReasoningConfiguration initial={reasoning.data!} /><GrafanaConfiguration initial={grafana.data!} /></div>}</section></div>
}
...
function ReasoningConfiguration({ initial }: { initial: ReasoningProfile }) {
  ...
  const payload = () => ({ provider: form.provider, base_url: form.base_url, model: form.model, credential_ref: form.credential_ref, json_mode: form.json_mode })
  const save = useMutation({ mutationFn: () => api<ReasoningProfile>('/api/v1/connections/reasoning/config', { method: 'PUT', body: JSON.stringify(payload()) }), onSuccess: (data) => { setForm(data); client.invalidateQueries({ queryKey: ['connections'] }) } })
  ...
  return <form className="connection-editor" onSubmit={(event) => { event.preventDefault(); save.mutate() }}><header><div><p className="eyebrow">Reasoning</p><h3>Provider and model</h3></div><Status value={form.credential_available ? 'credential_ready' : 'credential_missing'} /></header><Field label="Provider"><select value={form.provider} onChange={(e) => setForm({ ...form, provider: e.target.value })}><option value="mistral">Mistral</option><option value="openai-compatible">OpenAI-compatible</option></select></Field><Field label="API base URL"><input type="url" value={form.base_url} onChange={(e) => setForm({ ...form, base_url: e.target.value })} required /></Field><Field label="Model"><input value={form.model} onChange={(e) => setForm({ ...form, model: e.target.value })} required /></Field><Field label="Credential reference" hint="Environment variable name; the secret value is never stored."><input value={form.credential_ref} onChange={(e) => setForm({ ...form, credential_ref: e.target.value.toUpperCase() })} pattern="[A-Z][A-Z0-9_]{1,127}" required /></Field><label className="check-field"><input type="checkbox" checked={form.json_mode} onChange={(e) => setForm({ ...form, json_mode: e.target.checked })} /> Request provider JSON response mode</label><div className="button-row"><button className="primary" type="submit" disabled={save.isPending}>{save.isPending ? 'Saving…' : 'Save configuration'}</button><button type="button" disabled={test.isPending} onClick={() => test.mutate()}>{test.isPending ? 'Testing…' : 'Test connection'}</button></div><MutationFeedback mutation={save} success="Saved. Restart Triovexa to activate this provider." /><MutationFeedback mutation={test} success="Provider returned valid structured JSON." /></form>
}
