...
`cases.json` is the fixed 30-case set used to compare deterministic heuristics with a configured Mistral or OpenAI-compatible model. It contains ten worker stalls, eight timeout-after-deploy cases, six error-rate spikes, and six ambiguous cases that should produce no remediation.
...
go run ./cmd/evaluator -mode provider -provider mistral `
  -model mistral-small-latest -repeats 3 `
  -out evaluation/results/mistral-small-latest.json
```

For another Chat Completions endpoint, supply `-provider openai-compatible`, `-base-url`, and `-model`. Use the same cases, repeats, and prompt version when comparing reports.
